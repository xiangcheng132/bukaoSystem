      ...
      <div class="resources" v-for="(v,i) in props.singleList">
        ...
        <div class="opKS">
            <ul class="options">
              ...
              <div class="key" v-if="props.isAnswer">答案：<input type="text" :value="v.userKey" @change="updateKey(props.answerSheetId,v.id,$event.target.value)"></div>
              <div class="score" v-if="props.isAnswer">得分：{{v.score}}</div> 
              <div class="key" v-if="!props.isAnswer">答案：{{v.userKey}}</div>
              <div class="score" v-if="!props.isAnswer">得分：<input type="text" v-model="v.score" @change="updateKeyScore()"></div>  
         </div>
      </div>
        判断题：  
        <div class="resources" v-for="(v,i) in props.tFList">
          ...
          <div class="opKS">
            <ul class="options">
              ...
              <div class="score" v-if="props.isAnswer">得分：{{v.score}}</div>
              <div class="key" v-if="!props.isAnswer">答案：{{v.userKey}}</div>
              <div class="score" v-if="!props.isAnswer">得分：<input type="text" :value="v.score" @change="updateKeyScore()"></div>
          </div>
        </div>  
        填空题：
