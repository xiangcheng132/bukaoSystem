<template>
  ...
  <div>
    <el-table :data="examTableList" style="width: 100%">
      ...
      <el-table-column prop="place" label="考试地点" width="300" />
      <el-table-column prop="examClass" label="发布班级" width="300" />
      <el-table-column prop="createTime" label="开始时间" width="400" />
...
async function refreshTable(id) {
  ...
  await getExamInfoByTeaId(id).then((v) => {
    ...
    v.data.forEach((value) => {
      if (value.examClasses) {
        let result = "";
        value.examClasses.forEach((item) => {
          result = result + item.name + ",";
        });
        if (result.endsWith(",")) {
          result = result.substring(0, result.length - 1);
        }
        value["examClass"] = result;
      }
      examTableList.push(value);
    });
  });
  console.log(examTableList);
}
onMounted(() => {
  refreshTable(getStorage("token"));
});
